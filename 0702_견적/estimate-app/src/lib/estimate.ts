...
export type Estimate = {
  ...
  rates: EstimateRates
  nego: number
  takeoff?: import('./takeoff').Takeoff | null
  created_at: string
  updated_at: string
}
...
export const CATEGORY_LIST = ['학원/교습소', '뷰티/미용', '사무실', '식음료', '주거', '상업/기타', '대학/공공', '부분공사']

export const STATUS_LIST = ['작성중', '제출', '계약', '완료'] as const

export const UNIT_LIST = ['식', 'EA', 'M2', 'M', 'SET', '인', 'BOX', '롤', '단', '개소']

// 개략견적용 업종별 평당가 (VAT 포함, 2024~2026 본공사 44건 중앙값·범위)
// 출처: JM_견적통계_리포트_v3 (부분공사·추가공사·가구단독 제외, 90건 수동 분류)
export const PY_PRICE_STATS: Record<string, { median: number; min: number; max: number }> = {
  '뷰티/미용':   { median: 2_260_000, min: 1_490_000, max: 3_250_000 },
  '상업/기타':   { median: 1_650_000, min: 810_000, max: 2_710_000 },
  '사무실':     { median: 1_630_000, min: 690_000, max: 3_890_000 },
  '식음료':     { median: 1_590_000, min: 900_000, max: 4_510_000 },
  '학원/교습소': { median: 1_470_000, min: 590_000, max: 2_230_000 },
  '대학/공공':   { median: 1_280_000, min: 1_170_000, max: 1_390_000 },
}
...
export function calcGapji(sections: EstimateSection[], rates: EstimateRates, nego: number): GapjiCalc {
  ...
  // 갑지 표기는 '천단위절사'지만 실제 견적서 계산은 만단위 절사 (기존 견적서 검증 결과)
  const gross = Math.floor(grossBeforeCut / 10000) * 10000
