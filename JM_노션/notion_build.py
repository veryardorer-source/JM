...
def rel_oneway(target): return {'relation':{'database_id':target,'type':'single_property','single_property':{}}}
...
api('PATCH','/blocks/'+DASH+'/children',{'children':[div(),h2('🏗️ 진행중인 현장'),co('현장을 클릭하면 자료·공정·업무가 보입니다. 돈 관련 정보는 관리자 영역에서만 관리합니다. 새 현장은 + 새로 만들기.','🏗️','gray_background')]})
...
EXP=db(page(F_SITE,'📸 현장 경비·영수증','📸',[co('카드/현금 영수증 사진 업로드. 용도·현장 기록. 현장팀+관리팀.','📸','yellow_background')]),
  '현장 경비·영수증','📸',{
  '항목':{'title':{}},'현장':rel_oneway(SITE),'사용일':{'date':{}},'금액':won(),
...
WDR=db(page(F_SITE,'💸 출금 요청','💸',[co('일용직 임금·거래처 출금 요청. 현장팀·이사 올리면 대표 처리(결제=경리나라).','💸','yellow_background')]),
  '출금 요청 관리','💸',{
  '항목':{'title':{}},'현장':rel_oneway(SITE),'요청일':{'date':{}},'금액':won(),
...
ACC=db(page(F_MGR,'💰 매출·매입 (회계)','💰',[co('세부는 경리나라. 여기는 고정비·변동비·현장원가 흐름. 관리자 전용.','💰','red_background')]),
  ...
  '현장':rel_oneway(SITE),'결제수단':sel([opt('법인카드','blue'),opt('계좌이체','green'),opt('현금','gray')]),'증빙':{'files':{}},
})
FIN=db(page(F_MGR,'📊 현장별 재무','📊',[co('현장별 계약금액·총지출·수익. 관리자 전용.','📊','red_background')]),
  '현장별 재무','📊',{
  '현장명':{'title':{}},'현장':rel_oneway(SITE),'계약금액':won(),'총지출':won(),
...
print('=== 7. 관리자 재무 수익 공식 ===')
# 전 직원 현장 DB에는 금액 롤업/공식을 만들지 않는다.
# 경비·출금·회계·재무는 관리자 영역에서만 금액을 확인하고, 재무 총지출은 직접 입력한다.
...
row(FIN,{'현장명':T('오늘은 반찬데이 (예시)'),'현장':{'relation':[{'id':site1}]},'계약금액':N(28000000),'총지출':N(408000),'입금상태':S('🟡 계약금')})
